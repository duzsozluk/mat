...
def check_takeover(url):
  ...
  try:
    main_domain_ip = requests.get(f"https://check-host.net/{domain}").text.split("IP Address: ")[1].split(" ")[0]
  except:
  ...
  for subdomain in subdomains:
    try:
      subdomain_ip = requests.get(f"https://check-host.net/{subdomain}.{domain}").text.split("IP Address: ")[1].split(" ")[0]
    except:
